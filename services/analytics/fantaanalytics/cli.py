...
def run_scrape_transfermarkt(args: argparse.Namespace) -> int:
    if any(
        ...
        print(f"Profili riusciti: {summary.get('profiles_succeeded', summary['valid_players'])}")
        print(f"Profili falliti: {summary.get('profiles_failed', len(summary['errors']))}")
        print(f"Giocatori validi: {summary['valid_players']}")
        print(f"Record validi: {summary.get('records_valid', summary['valid_players'])}")
        print(f"Record importati: {summary.get('records_imported', summary['inserted_players'] + summary['updated_players'])}")
